...
datapath = "G:\\Dev\\Data\\100000\\GNPS Python 100000 Binned"
filtered_datapath = "G:\\Dev\\Data\\100000\\GNPS Python 100000 Filtered"

double_filtered_datapath = "G:\\Dev\\Data\\100000\\GNPS Python 100000 Double Filtered"

fragment_id_datapath = "G:\\Dev\\Data\\100000\\GNPS Python 100000 With Fragments"
...
def top_six_filter(spectrum):
    ...
    for i in range(len(spectrum)):  # For each mass bin
        ...
        if i >= 5000:
            low_end = 5000  # Else, go back 5000 indices
        # Compare value to all bins in 10000Da range
        window_comparison = np.less(spectrum[i], spectrum[i-low_end:(i+5000)])
        if np.sum(window_comparison) < 7:  # If value is among top 6 in 10000Da range, add it to filtered array.
...
def scan_for_fragments(spectrum):
    ...
    for i, fragment in enumerate(fragments):
        f_mass = round(float(fragment['mass']), 2)
        f_mass_index = int(f_mass * 100)

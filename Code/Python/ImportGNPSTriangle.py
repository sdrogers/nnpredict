...
binned_datapath = "G:\\Dev\\Data\\1000_Triangle\\GNPS Python Binned"
...
def calculate_value(mass, intensity, bin_size=1):
    ...
    if mass == 1:
        ...
        lower_mass_bin = int(mass) - 1
        upper_mass_bin = int(mass) + 1
        lower_mass_bin_intensity = 0
        upper_mass_bin_intensity = 0
    ...
    if mass > 1 and mass not in range(2, 1000):
        lower_mass_bin = (math.floor(float(mass)) // bin_size) - 1
    if mass < 1000 and mass not in range (2, 1000):
        upper_mass_bin = (math.ceil(float(mass)) // bin_size) - 1

    if upper_mass_bin is not None:
        lower_mass_bin_intensity = (-(mass - 1) + upper_mass_bin) * intensity
    if lower_mass_bin is not None:
        upper_mass_bin_intensity = ((mass - 1) - lower_mass_bin) * intensity
